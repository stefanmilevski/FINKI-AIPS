...
public class ArrayMeanValue {
    ...
    public static int brojDoProsek(Array<Integer> arr) {
        ...
        double differenceToAverage = Double.MAX_VALUE;
        int closestNumberIndex = 0;

        for(int j = 0; j < arr.getSize();j++){
            if(Math.abs(arr.get(j) - average) < differenceToAverage){
                differenceToAverage = Math.abs(arr.get(j) - average);
                closestNumberIndex = j;
            }
            if(Math.abs(arr.get(j) - average) == differenceToAverage && arr.get(j) < arr.get(closestNumberIndex)) {
                closestNumberIndex = j;
            }
        }

        return arr.get(closestNumberIndex);
    }
